...
parameter_names_long = ['Min','Dry state mu', 'Dry state sigma', 'Wet state mu', 
...
def plotSDC(synthetic, histData, structure_name):
    ...
    fig.savefig('../../../'+design+'/MultiyearShortageCurves/' + structure_name + '.svg')
    fig.savefig('../../../'+design+'/MultiyearShortageCurves/' + structure_name + '.png')
    fig.clf()
    ...
    bottom_row = pd.DataFrame(data=np.array([np.zeros(100)]), index= ['Interactions'], columns=list(delta_values.columns.values))
    ...
    bottom_row = pd.DataFrame(data=np.array([np.zeros(100)]), index= ['Interactions'], columns=list(S1_values.columns.values))
    ...
    for p in percentiles:
        ...
        if total!=0:
            diff = 1-total
            S1_values.set_value('Interactions',str(p),diff)
            for param in param_names+['Interactions']:
    ...
    bottom_row = pd.DataFrame(data=np.array([np.zeros(100)]), index= ['Interactions'], columns=list(R2_values.columns.values))
    ...
    for p in percentiles:
        ...
        if total!=0:
            diff = 1-total
            R2_values.set_value('Interactions',str(p),diff)
            for param in param_names+['Interactions']:
    ...
    for k in range(len(titles)):
        ...
        ax1.set_ylabel('Annual shortage (af)', fontsize=12)
        ax1.set_xlabel('Shortage magnitude percentile', fontsize=12)
        plt.legend(handles[1:], labels = parameter_names_long[1:], fontsize=10, loc='lower center',ncol = 6)
        plt.subplots_adjust(bottom=0.2)
...
for i in range(start, stop):
    #getinfo(all_IDs[i])
    histData = np.loadtxt('../../../'+design+'/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_hist.txt')[:,2]
