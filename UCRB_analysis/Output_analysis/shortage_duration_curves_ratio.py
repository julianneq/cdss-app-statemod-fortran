...
#all_IDs = np.genfromtxt('../Structures_files/metrics_structures.txt',dtype='str').tolist() #irrigation_structures_flat+WDs+non_irrigation_structures
all_IDs = ['7200645']
...
#        ax1.fill_between(p_i, np.nanmin(multi_year_durations_percentiles[:,:],1), np.percentile(multi_year_durations_percentiles[:,:], p[i], axis=1), color=color, alpha = 0.1)
...
#    fig.clf()
    ...
    color = '#000292'
    for i in range(len(p)):
        ax1.fill_between(P, np.nanmin(F_syn[:,:],1), np.percentile(F_syn[:,:], p[i], axis=1), color=color, alpha = 0.1)
...
for i in range(len(all_IDs)):
    if all_IDs[i] in WDs:
        ...
        for ID in irrigation_structures[WDs.index(all_IDs[i])]:
            histData_shortage += np.loadtxt('../../../LHsamples_original_1000_AnnQonly/Infofiles/' +  ID + '/' + ID + '_info_hist.txt')[:,2]
            histData_demand += np.loadtxt('../../../LHsamples_original_1000_AnnQonly/Infofiles/' +  ID + '/' + ID + '_info_hist.txt')[:,1]
            for j in range(samples):
                for r in range(realizations):
                    data_s= np.loadtxt('../../../' + design + '/Infofiles/' +  ID + '/' + ID + '_info_' + str(j+1) + '.txt')[:,2*(r+1)] 
                    data_d= np.loadtxt('../../../' + design + '/Infofiles/' +  ID + '/' + ID + '_info_' + str(j+1) + '.txt')[:,2*r+1] 
                synthetic_shortage[:,j,r]+=data_s
                synthetic_demand[:,j,r]+=data_d
    else:
        histData_shortage = np.loadtxt('../../../LHsamples_original_1000_AnnQonly/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_hist.txt')[:,2]
        histData_demand = np.loadtxt('../../../LHsamples_original_1000_AnnQonly/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_hist.txt')[:,1]
        #uncurt_shortage = np.loadtxt('../../../' + design + '/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_hist_uncurtailed.txt')[:,2]
        #uncurt_demand = np.loadtxt('../../../' + design + '/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_hist_uncurtailed.txt')[:,1]
        synthetic_shortage = np.zeros([len(histData_shortage),samples, realizations])
        synthetic_demand = np.zeros([len(histData_shortage),samples, realizations])
        for j in range(samples):
            for r in range(realizations):
                data_s= np.loadtxt('../../../' + design + '/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_' + str(j+1) + '.txt')[:,2*(r+1)]     
                data_d= np.loadtxt('../../../' + design + '/Infofiles/' +  all_IDs[i] + '/' + all_IDs[i] + '_info_' + str(j+1) + '.txt')[:,2*r+1]     
                synthetic_shortage[:,j,r]=data_s
                synthetic_demand[:,j,r]=data_d
