...
all_IDs = np.genfromtxt('../Structures_files/non_flow_structures.txt',dtype='str').tolist()
...
np.save('RobustnessRanks_no15mi.npy',allRobustness)


designs = ['LHsamples_original_100_AnnQonly','LHsamples_wider_100_AnnQonly','CMIPunscaled_SOWs','Paleo_SOWs']
allRobustness = np.zeros([10,10,nStructures,len(designs)])
for i, design in enumerate(designs):
	for j, ID in enumerate(all_IDs):
		allSOWs = np.load('../../../'+design+'/Factor_mapping/'+ ID + '_heatmap.npy')
...
np.save('RobustnessRanks_no15mi_100samples.npy',allRobustness)
