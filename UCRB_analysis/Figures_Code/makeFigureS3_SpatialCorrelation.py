...
def makeFigureS3_SpatialCorrelation():

    MonthlyQ_s = np.load('../Qgen/Sample1_Flows_logspace.npy') # nyears x nsites x 12
    MonthlyQ_s = np.reshape(np.swapaxes(MonthlyQ_s,1,2),[105*12,208]) # nyears*12 x nsites
    
    MonthlyQ_h = np.loadtxt('../Qgen/MonthlyQ.csv',skiprows=1,usecols=np.arange(1,209),delimiter=',') # nyears*12 x nsites
