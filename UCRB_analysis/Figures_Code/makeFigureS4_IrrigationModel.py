...
def makeFigureS4_IrrigationModel():
    
    AnnualIWR = np.loadtxt('../Qgen/AnnualIWR.csv',delimiter=',')*1233.48 # convert to m^3
    AnnualQ = np.array(pd.read_csv('../Qgen/AnnualQ.csv'))
    ...
    ax.set_xlabel('Annual Flow Anomaly at CO-UT State Line (m' +r'$^3$' + ')',fontsize=16)
    ax.set_ylabel('Annual Total Irrigation Demand Anomaly (m' +r'$^3$' + ')',fontsize=16)
