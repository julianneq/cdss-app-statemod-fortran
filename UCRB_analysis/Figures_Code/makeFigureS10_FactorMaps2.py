...
def makeFigureS10_FactorMaps2():
    ...
    for percentile in percentiles:
        ax.plot([percentile, percentile],[0,370000000],c='k')
    ...
    addPercentileBlocks(historic_percents, gridcells, percentiles, short_magnitudes)
    ...
    for i in range(len(percentiles)):
        for j in range(len(short_magnitudes)):
            dta['Success'] = allSOWsperformance[list(frequencies).index(100-percentiles[i]),int(short_magnitudes[j]/10)-1,:]
            pseudo_r_scores = all_pseudo_r_scores[str((100-percentiles[i]))+'yrs_'+str(short_magnitudes[j])+'prc'].values
            print(pseudo_r_scores)
            if pseudo_r_scores.any():
                ax = fig.add_subplot(2,3,i*3+j+2)
                ...
                result = fitLogit(dta, [all_predictors[1]])
                print(result.prsquared)
                try:
                    print("interact")
...
def addPercentileBlocks(historic_percents, gridcells, percentiles, short_magnitudes):   
    for i in range(len(percentiles)):
        for j in range(len(short_magnitudes)):
            highlight_cell(int(short_magnitudes[j]/10)-1, int((100-percentiles[i])/10)-1, color="black", linewidth=2)
