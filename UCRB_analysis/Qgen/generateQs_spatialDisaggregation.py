...
def generateQs_spatialDisaggregation(LHsamples_file):
	...
	LHsamples = np.loadtxt(LHsamples_file,usecols=[0,7,8,9,10,11,12,13])
	...
	MonthlyQ_h = pd.read_csv('MonthlyQ.csv', index_col=0).values
	AnnualQ_h = pd.read_csv('AnnualQ.csv', index_col=0).values
	...
	for i in range(start, stop):
	    for realization in range(10):
	       # create matrices to store nYears of synthetic (_s), real-space (AnnualQ_s), 
	        ...
	        writeNewFiles('cm2015x.xbm', 16, i, realization+1, MonthlyQ_s, LHsamples_file)
	        
	        # write new irrigation demands to file for LHsample i
	        writeNewFiles('cm2015B.iwr', 463, i, realization+1, MonthlyIWR_s, LHsamples_file) # a for all uncertainties

	return None

LHsamples_file = str(sys.argv[1])
generateQs_spatialDisaggregation(LHsamples_file)