...
# Fit HMM to Paleo data
# load paleo data at Cisco
Paleo = pd.read_csv('Reconstruction/Cisco_Recon_v_Observed_v_Stateline.csv')
...
def fitParams(flows, title):
    ...
    assessFit(np.log(flows), hidden_states, mus, sigmas, P, title, 'Recon')
...
# fit HMM to reconstructed flows
paleoParams = fitParams(np.array(Paleo['ScaledReconCisco'][340:429]), '1909-1997')
print(paleoParams)

# add noise to reconstructed flows and fit HMM
# repeat 100x and report mean parameter estimates
nsims = 100
...
for i in range(nsims):
    ...
    simParams[i,:] = fitParams(np.array(flows), 'Simulation ' + str(i+1))
