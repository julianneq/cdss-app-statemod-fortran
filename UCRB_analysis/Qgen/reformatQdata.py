...
def writeNewFiles(filename, firstLine, numSites):
    ...
    # calculate shifted annual flows
    ShiftedAnnualQ = np.zeros([numYears-1,numSites])
    for i in range(numYears-1):
        ShiftedAnnualQ[i,:] = np.sum(MonthlyQ[i*12+6:(i+1)*12+6])
        
    np.savetxt('ShiftedAnnualQ.csv',ShiftedAnnualQ,fmt='%d',delimiter=',')
