...
def fitHMM(logAnnualQ):
    ...
    for i in range(np.shape(logAnnualQ)[1]):
        # fit to last 2/3 of historical record
        hmm_model = GaussianHMM(n_components=2, n_iter=1000).fit(np.reshape(logAnnualQ[35::,i],[len(logAnnualQ[35::,i]),1]))
